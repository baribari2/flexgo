...
func (f *FC) Start(wg *sync.WaitGroup, ec *ethclient.Client, senderPK, senderA string) {
	...
	if err != nil {
		log.Printf("failed to subscribe to new head: %v", err.Error())
		return
	}
	...
	for h := range ch {
		...
		res, err := ec.CallContract(context.Background(), geth.CallMsg{
		...
		if err != nil {
			log.Printf("failed to call contract: %v", err.Error())
			continue
		}
		...
		_, err = types.SignTx(txn, types.NewEIP155Signer(cid), pk)
		if err != nil {
			log.Printf("failed to sign transaction: %v", err.Error())
			return
		}
		// -------------------- Transaction -------------------- //
	}
